...
if ($result && mysqli_num_rows($result) > 0) {
    ...
    $vehicleQuery = "SELECT vcl_no, vcl_make, vcl_model FROM vehicles WHERE nic = '$nic'";
    ...
    if ($vehicleResult && mysqli_num_rows($vehicleResult) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($vehicleResult)) {
            ...
            $vehicleMake = $row['vcl_make'];
            $vehicleModel = $row['vcl_model'];

            // Add the vehicle details to the array
            $vehicles[] = array(
                'vcl_no' => $vehicleNumber,
                'vcl_make' => $vehicleMake,
                'vcl_model' => $vehicleModel
            );
        }
